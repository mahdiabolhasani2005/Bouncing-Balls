...
#ifndef BP_PROJECT_GAME_UNLIMITED_H
#define BP_PROJECT_GAME_UNLIMITED_H
int Game_unlimited (vector<user> users,int row = 20)
{
    ...
    while (!Quit) {
        if (!Pause) {
            ...
            if (BF.ball0.y > -100){
                BF.CreateBallFieldapp(20,20,valid,Explode,Exploding,FB);
            }
            for (int i = 0; i < BF.Balls[0].size(); ++i) {
                if (BF.Balls[0][i] > 0)
...
#endif //BP_PROJECT_GAME_UNLIMITED_H
